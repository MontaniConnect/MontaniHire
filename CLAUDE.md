...
- The Dockerfile does `COPY vendor/* ./vendor/` so vendored JS is included in the built image; Propshaft serves it without a separate `assets:precompile` step
- `railway.toml` currently has two sequential preDeployCommands: `bundle exec rails assets:precompile` followed by `bundle exec rails db:migrate` — `assets:precompile` was added as a precaution but the confirmed fix for Trix was vendoring the ESM build; whether precompilation is strictly necessary with Propshaft has not been conclusively tested
